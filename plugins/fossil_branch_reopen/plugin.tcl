...
namespace eval ::plugins::fossil_branch_reopen {}

proc ::plugins::fossil_branch_reopen::execute {
    ...
    if {[dict exists $settings max_name_chars]} {
        set maximum [::plugins::fossil_common::positiveInteger \
            ...
            "fossil_branch_reopen max_name_chars setting"]
    }
    set name [dict get $arguments name]
    if {[string trim $name] eq ""} {
        error "fossil_branch_reopen name must not be empty"
    }
    if {[string length $name] > $maximum} {
        error "fossil_branch_reopen name exceeds configured limit: $maximum"
    }
