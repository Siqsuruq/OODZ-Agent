...
proc ::oodzGui::approve {name arguments} {
    ...
    ttk::button .approval.yes -text "Yes" \
        -command [list ::oodzGui::approvalChoice 1]
    if {$name ni {run_tcl_file run_project_tests}} {
        ttk::button .approval.all -text "All for session" \
            -command [list ::oodzGui::approvalChoice 2]
    }
    ttk::button .approval.no -text "Deny" \
        -command [list ::oodzGui::approvalChoice 0]
    grid .approval.message -row 0 -column 0 -columnspan 3 \
        -sticky nsew -padx 16 -pady {16 20}
    grid .approval.yes -row 1 -column 0 -padx {16 4} -pady {0 16} \
        -sticky ew
    if {$name ni {run_tcl_file run_project_tests}} {
        grid .approval.all -row 1 -column 1 -padx 4 -pady {0 16} \
            -sticky ew
    }
    grid .approval.no -row 1 -column 2 -padx {4 16} -pady {0 16} \
        -sticky ew
    grid rowconfigure .approval 0 -weight 1
    foreach column {0 1 2} {
        grid columnconfigure .approval $column -weight 1
    }
    wm protocol .approval WM_DELETE_WINDOW \
        [list ::oodzGui::approvalChoice 0]
...
proc ::oodzGui::renderHistory {} {
    ...
    foreach message [$agent getHistory] {
        if {![dict exists $message content]
                || [string trim [dict get $message content]] eq ""} {
            continue
        }
        ...
        ::oodzGui::appendMessage \
            [expr {$role eq "user" ? "You" : "Assistant"}] \
...
proc ::oodzGui::showSkills {} {
    ...
    ttk::scrollbar .skills.scroll -orient vertical \
        -command [list .skills.list yview]
...
proc ::oodzGui::selectTool {} {
    ...
    .tools.right.description configure \
        -text [dict get $definition description]
...
proc ::oodzGui::showTools {} {
    ...
    ttk::scrollbar .tools.left.namesScroll -orient vertical \
        -command [list .tools.left.names yview]
    .tools.left.names configure \
        -yscrollcommand [list .tools.left.namesScroll set]
    ttk::label .tools.right.description -text "Select a tool" \
        -anchor nw -justify left -wraplength 500
    ttk::label .tools.right.schemaLabel -text "Argument schema"
    text .tools.right.schema -height 5 -wrap word -state disabled \
        -padx 8 -pady 8
    ...
    text .tools.right.result -height 10 -wrap word -state disabled \
        -padx 8 -pady 8
    .tools.right.result tag configure error -foreground #c62828
    ttk::button .tools.right.run -text "Run tool" -state disabled \
        -command ::oodzGui::runSelectedTool
    ttk::button .tools.right.close -text "Close" \
        -command [list destroy .tools]
    ...
    grid .tools.left.namesScroll \
        -row 0 -column 1 -sticky ns
    grid .tools.right.description \
        -row 0 -column 0 -columnspan 2 -sticky ew -pady {0 10}
    grid .tools.right.schemaLabel \
        -row 1 -column 0 -columnspan 2 -sticky w
    grid .tools.right.schema \
        -row 2 -column 0 -columnspan 2 -sticky nsew -pady {4 10}
    grid .tools.right.argumentsLabel \
        -row 3 -column 0 -columnspan 2 -sticky w
    grid .tools.right.arguments \
        -row 4 -column 0 -columnspan 2 -sticky nsew -pady {4 10}
    grid .tools.right.resultLabel \
        -row 5 -column 0 -columnspan 2 -sticky w
    grid .tools.right.result \
        -row 6 -column 0 -columnspan 2 -sticky nsew -pady {4 10}
...
proc ::oodzGui::applyConfiguredTheme {configObject} {
    ...
    if {$themePackage ne ""} {
        foreach packageDir [glob -nocomplain -types d -directory \
                [file join $scriptDir lib] *] {
...
proc ::oodzGui::applyTextTheme {} {
    ...
    if {$darkTheme} {
        .input configure -background #404452 -foreground #d3dae3 \
            -insertbackground white
    } else {
        .input configure -background white -foreground #5c616c \
...
proc ::oodzGui::showTextContextMenu {widget rootX rootY localX localY} {
    ...
    .textContext entryconfigure Copy \
        -state [expr {$hasSelection ? "normal" : "disabled"}]
    .textContext entryconfigure Cut \
        -state [expr {$editable && $hasSelection ? "normal" : "disabled"}]
    .textContext entryconfigure Paste \
        -state [expr {$editable && $hasClipboard ? "normal" : "disabled"}]
    .textContext entryconfigure "Select All" \
        -state [expr {$hasText ? "normal" : "disabled"}]
...
proc ::oodzGui::buildWidgets {} {
    ...
    text .conversation -wrap word -state disabled -padx 12 -pady 12 \
        -background #151821 -foreground #d8dee9 -insertbackground white \
        -selectbackground #5294e2 -selectforeground white \
        -relief flat
    ttk::scrollbar .scroll -orient vertical \
        -command [list .conversation yview]
    ...
    text .input -height 5 -wrap word -padx 8 -pady 8 \
        -selectbackground #5294e2 -selectforeground white
    ttk::frame .actions
    ...
    ttk::checkbutton .themeToggle -text "Dark" \
        -variable ::oodzGui::darkTheme -command ::oodzGui::toggleTheme
    if {"Arc" ni [ttk::style theme names]
            || "Arc-Dark" ni [ttk::style theme names]} {
        .themeToggle configure -state disabled
    }
    menu .textContext -tearoff 0
    .textContext add command -label Cut \
        -command [list ::oodzGui::contextAction <<Cut>>]
    .textContext add command -label Copy \
        -command [list ::oodzGui::contextAction <<Copy>>]
    .textContext add command -label Paste \
        -command [list ::oodzGui::contextAction <<Paste>>]
    .textContext add separator
    .textContext add command -label "Select All" \
        -command ::oodzGui::contextSelectAll
    ...
    grid .input -in .main -row 1 -column 0 -columnspan 2 \
        -sticky ew -pady {10 8}
    grid .actions -in .main -row 2 -column 0 -columnspan 2 -sticky ew
    pack .send .new .toolsButton .skillsButton \
        -in .actions -side left -padx {0 8}
...
proc ::oodzGui::start {} {
    ...
    [::tLogger getLogger "Global"] setLogLevel \
        [$config get Logging.level info]
    set workspaceRoot [::resolveWorkspaceRoot \
        $scriptDir [$config get Workspace.root .]]
    set instructions [::loadWorkspaceInstructions \
    ...
    set agent [tAgent new \
        ...
        [::buildAgentSystemRole \
            $config $instructions [$skillRegistry summaries] \
